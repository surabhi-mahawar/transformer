package com.samagra.transformer.odk.openrosa.okhttp;


import com.samagra.transformer.odk.utilities.FileUtils;
import lombok.extern.slf4j.Slf4j;
import com.samagra.transformer.odk.openrosa.CaseInsensitiveEmptyHeaders;
import com.samagra.transformer.odk.openrosa.CaseInsensitiveHeaders;
import com.samagra.transformer.odk.openrosa.HttpCredentialsInterface;
import com.samagra.transformer.odk.openrosa.HttpGetResult;
import com.samagra.transformer.odk.openrosa.HttpHeadResult;
import com.samagra.transformer.odk.openrosa.HttpPostResult;
import com.samagra.transformer.odk.openrosa.OpenRosaHttpInterface;
import com.samagra.transformer.odk.openrosa.OpenRosaServerClient;
...
public class OkHttpConnection implements OpenRosaHttpInterface {
    ...
    public HttpHeadResult executeHeadRequest(@NonNull URI uri, @Nullable HttpCredentialsInterface credentials) throws Exception {
        ...
        log.info("Issuing HEAD request to: %s", uri.toString());
        Response response = httpClient.makeRequest(request, new Date());
        int statusCode = response.code();
    ...
    public HttpPostResult uploadSubmissionFile(@NonNull List<File> fileList, @NonNull File submissionFile, @NonNull URI uri, @Nullable HttpCredentialsInterface credentials, @NonNull long contentLength) throws Exception {
        ...
        while (fileIndex < fileList.size() || first) {
            ...
            log.info("added xml_submission_file: %s", submissionFile.getName());
            ...
            for (; fileIndex < fileList.size(); fileIndex++) {
                ...
                log.info("added file of type '%s' %s", contentType, file.getName());
