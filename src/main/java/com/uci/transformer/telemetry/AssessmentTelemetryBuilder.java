...
public class AssessmentTelemetryBuilder {
	...
	public String build(String botOrg, String channel, String provider, String producerID, String conversationOwnerID,
			Question question, Assessment assessment, long duration) {
		...
		Context context = Context
				.builder().channel(channelName).env(channel + "." + provider).pdata(Producer.builder()
						.id("prod.uci.diksha").pid(producerID).ver(QUESTION_TELEMETRY_IMPL_VERSION).build())
				.did(userID).cdata(cdata).rollup(rollup).build();
...
//			e.printStackTrace();
		...
		Telemetry telemetry = Telemetry.builder().eid(TelemetryEvents.ASSESS.getName()).ets(System.currentTimeMillis())
				.ver(TELEMETRY_IMPL_VERSION).mid(ASSESS_EVENT_MID)
				.actor(Actor.builder().type(ACTOR_TYPE_USER).id(userID).build()).context(context).object(object)
				.edata(edata).build();
